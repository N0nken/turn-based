...
func plan_action(action : TB_Action) -> bool:
	if action.cost + _used_turn_plan_capacity <= weapon.turn_plan_capacity and _planned_turns.size() < MAX_TURNS:
...
func unplan_latest_turn() -> void:
	if _planned_turns.size() == 0:
		return
	var action : TB_Action = _planned_turns.pop()
	_used_turn_plan_capacity -= action.cost
