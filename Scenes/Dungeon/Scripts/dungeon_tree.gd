...
func _generate_nodes(template : DungeonTemplate) -> void:
	...
	total_depth = randi_range(template.tree_depth.min, template.tree_depth.max)
	...
	for depth in range(1,total_depth+1):
		...
		if depth < total_depth * 2 / 3:
		...
		elif depth >= total_depth * 2 / 3 and depth != total_depth+1:
			var minimum_change := 0
			...
			total_new_nodes = 1
		if total_new_nodes <= 0 and depth != total_depth+1:
			total_new_nodes *= -1 + 1
...
func next_node(current_node : DungeonNode) -> void:
	...
	current_node.clear()
	current_node.activate_children()
	for locked_node in get_node("DungeonNodes").get_child(current_node.depth).get_children():
		if not locked_node.cleared:
			locked_node.lock()
	if current_node.branches.size() > 0:
		current_node.branches[0].get_node("Button").grab_focus()
	else:
